...
given the following directions of $x$ and $y$:
\begin{center}
\carloTikZ{\carloXy
\foreach \x in {0,1}
\foreach \y in {0,1}
...
The goal is to make $u[\lcon/x, \rcon/y]\equiv a$ and $u[\rcon/x]\equiv v$.
From the goal we may deduce $\vdash a:A$ and $y:\II\vdash v:A$.
...
\begin{center}
\carloTikZ{\carloXyz \refcube{Empty}}
\end{center}
\item $\cond$ specifies a $2$-cube (a square) in $\Phi$.
\item $\conj$ specifies any $n$-cube (for $n\leq 2$) in $\Phi$. For example, $x=\lcon\land y=\rcon$ specifies a line,
