...
\begin{figure}[h!]
\begin{align*}
\Phi;\Gamma,\overline{\conj}&\vdash u\equiv v:A\\
\Phi;\Gamma,\overline{\disj}&\vdash u\equiv v:A\\
\Phi;\Gamma,\overline{\conj}&\vdash u:A\\
\Phi;\Gamma,\overline{\conj}&\vdash \isType{A\equiv B}\\
\Phi;\Gamma,\overline{\conj}&\vdash \isType{A}\\
\Phi;\Gamma,\overline{\disj}&\vdash \disj_0\equiv \disj_1
\end{align*}
...
\subsection{Transportation rules and cubical subtypes}\label{sub:transp}
...
\item If there is a cofibration $\disj$ such that $A$ does not refer to $x$ under $\disj$.
...
\item It computes on introduction rules.
