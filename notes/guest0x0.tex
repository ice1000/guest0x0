...
There are two versions of composition: \textit{homogeneous composition} and
...
\item[Glue type] is a type former (with an introduction and elimination rule)
  that is similar to \hcompLong{} on the universe, but with the edges replaced
	...
	Glue type can be used to prove univalence, and is discussed in~\cite{CCHM,HCompPDF}.
\item[V type] is a special case of Glue type that is sufficient to prove univalence,
  but cannot handle \hcompLong{} on universes, hence a \CTT{} with V type needs
	...
	V type is discussed in~\cite{ABCFHL}.
...
\item[Cubical computational type theory] is a variant of \CTT{} built in a computational
  (extensional) type theory, discussed in~\cite{CHTT1,CHTT2,CHTT3,CHTT4}.
	...
	that the is not going to work well
	due to the lack of automated $\eta$-equality support.
\item[Higher inductive types] are a way to define inductive types.
  They are introduced in~\cite{CHM} with a general scheme
	(attach a partial element on a constructor).
	Higher inductive types in Cartesian cubical computational type theory are discussed in~\cite{HIT-CCTT}.
\end{description}
