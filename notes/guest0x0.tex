...
\begin{mathpar}
\inferrule{\Psi,x:\II;\isType A\\ \PGvdash a:A[\lcon/x]\\ \PGvdash b:A[\rcon/x]}
{\PGvdash\isTypeBox{\PathTy{\lam x A} a b}}\and
\inferrule{\Psi,x:\II;\Gvdash u:A}
{\PGvdash \plam x u : \PathTy{\lam x A}{(u[\lcon/x])}{(u[\rcon/x])}}\and
\inferrule{\Hint{\Psi,x:\II;\isType A}\\\Psi,x:\II;\Gvdash u:A\\\PGvdash v:\II}{
\PGvdash \papp{(\plam x u)}v \equiv u[v/x]:A[v/x]}\and
\inferrule{\Hint{\Psi,x:\II;\isType A}\\\PGvdash u:\PathTy{\lam x A} a b}{
...
\begin{itemize}
\item \fbox{$\Psi,x:\II;\Gvdash u:A$}, just like checking \fbox{$\lam x u:(x:\II)\to A$}.
...
\[\Psi,x:\II,\Gamma,\disj\vdash \transpLong(u)\equiv u:A[\lcon/x]\]
...
\begin{figure}[h!]
\[u,A::= \transp{A}{\disj} \mid \cdots~\text{(see~\cref{fig:subtype})}\]
...
\[\transpLong:(A:\II\to\mathbb{U})~(\psi:\II)~(A~\lcon)\to(A~\rcon)\]
