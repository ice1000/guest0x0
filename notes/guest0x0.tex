...
\begin{figure}[h!]
\[u,A::= \hcomp A u \mid \cdots~\text{(see~\cref{fig:syntax-transp})}\]
...
\PGvdash{\disj} \\
\PGvdash u : \II \to \Partial{\disj} A}{
\hcomp{A}{u}:\subTyImpl{A}{\disj}{(u~\rcon)}}
\end{mathpar}
...
: A~r \to A~\rcon} \and
\inferrule{\PGvdash A:\II\to\UU \\
\PGvdash u : (x:\II) \to \Partial{\disj} {(A~x)}}
{\textsf{comp}_A}
\end{mathpar}
\end{defn}
...
\begin{prop}
\[\forall u,	\textsf{forward}_A^\rcon(u)\equiv u\]
\end{prop}

\begin{terminology}
A type former is called \textit{fibrant} if it supports Kan operations
