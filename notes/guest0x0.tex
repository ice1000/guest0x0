...
In \CTT, for every line on types \fbox{$\PiGvdash x\isType{A}$},
we add a function $\coeLong:A[\lcon/x]\to A[\rcon/x]$ to the type theory,
satisfying the following properties:
\begin{itemize}
\item It is an isomorphism -- so we want
an inverse $\coeLong_A^{-1}:A[\rcon/x]\to A[\lcon/x]$.
One may easily guess that this is achieved by the
$\coeLong$ function on type $A[\neg x/x]$.
\item There exists a cofibration $\disj$ such that $A$ does not reference $x$ under $\disj$.
We may apply the following check:
\[\Psi,\Gamma,\disj\vdash \isTypeBox{A[\lcon/x]\equiv A[\rcon/x]}\]
As in~\cite[\S 2.1]{HCompPDF}, we may also check:
\[\Psi,x:\II,\Gamma,\disj\vdash \isTypeBox{A\equiv A[\lcon/x]}\]
Furthermore, we want that for every $\PGvdash u:A[\lcon/x]$:
\[\Psi,x:\II,\Gamma,\disj\vdash \coeLong(u)\equiv u:A[\lcon/x]\]
...
The implementation is basically a rewrite of~\cite[\S 2, \S 3]{HCompPDF},
as in~\cref{fig:typing-transp}.

\begin{figure}[h!]
\begin{mathpar}
\inferrule{\PiGvdash{x}\isType A \\
\Psi\vdash\overline{\conj_i} \\ \PiGvdash{x}\isTypeBox{A\equiv A[\lcon/x]}}{
\coe{\lam x A}{\overline{\conj_i}}:(u:A[\lcon/x]) \to
 \subTy{A[\rcon/x]}{\overline{\conj_i}}{\overline{\conj_i\mapsto u}}}
\end{mathpar}
...
\begin{mathpar}
\inferrule{\PiGvdash{x}\isType A \\
\Psi\vdash\disj \\ \PiGvdash{x}\isTypeBox{A\equiv A[\lcon/x]}}{
\PGvdash\coe{\lam x A}{\disj}:A[\lcon/x] \to A[\rcon/x]}\and
\inferrule{\PiGvdash{x}\isType A \\ \Psi\vdash\disj}{
\Psi;\Gamma,\disj\vdash\coe{\lam x A}{\disj}\equiv \lam x x:A[\lcon/x] \to A[\lcon/x]}
\end{mathpar}
...
\begin{lem}[Fill]
Given a well-typed term \[\coe{\lam x A}{\disj}{(u)}\]
There exists \[p:\ExtTy y {A[y/x]}{ccl}{
	x&=\rcon&\mapsto \coe{\lam x A}{\disj}{(u)} \\
	x&=\lcon&\mapsto u}\]
\end{lem}
\begin{proof}
Let
\[p:=\plam x{\coe{\lam y{A[x\land y/x]}}
 {\conj\land x=\lcon}(u)}\]
...
very long and are already written in~\cite[\S 3]{HCompPDF}.
