...
\begin{figure}[h!]
\[u,A::= \extTy{\overline{x_i}}{A}{\overline{\face}}
...
\begin{figure}[h!]
\begin{mathpar}
\inferrule{\Phi\overline{,x_i:\II};\Gvdash \isType{A}\\
\PGvdash\lrbbar{\overline{\conj_i\mapsto u_i}}
:\Partial{\left(\bigvee\nolimits_{i\in I} \conj_i\right)}A}
{\TODO}
\end{mathpar}
...
\[\textsf{concat}~(a:A)~(b:A)~(c:A)~(p:\PathTy A a b)~(q:\PathTy A b c):\PathTy A a c\]
...
\begin{align*}
\textsf{concat}~&(p:\extTy{x}{A}{})~(q:\extTy{x}{A}{x=\lcon\mapsto p~@~\rcon})\\
&:{\ExtTy{x}{A}{rll}{x&=\lcon&\mapsto p~@~\lcon\\
