...
like \fbox{$x\equiv\lcon$} or \fbox{$x\equiv\rcon$}, as in~\cref{fig:cofib} (recall~\cref{not:syntax-def}).
\begin{figure}[h!]
\[\begin{array}{rll}
...
and the face becomes $y\equiv\lcon\mapsto u$.
\item $\lcon$, i.e. take the face that $x\equiv\lcon$ in this partial element.
...
This happens when for a variable $x$, we have both $x\equiv\lcon\in\conj$
and $x\equiv\rcon\in\conj$. In this case, we also remove the face.
\item It can contain duplicated information.
For example, we may write $x\equiv\lcon\land x\equiv\lcon$.
...
:\Partial{\left(\bigvee_{i\in I} \conj_i\right)}A}
\end{mathpar}
...
{\PGvdash \plam x u : \PathTy A{(u[\lcon/x])}{(u[\rcon/x])}}\and
\inferrule{\Hint{\Phi,x:\II;\isType A}\\\Phi,x:\II;\Gvdash u:A\\\PGvdash v:\II}{
\PGvdash (\plam x u)~@~v \equiv u[v/x]:A[v/x]}\and
\inferrule{\Hint{\Phi,x:\II;\isType A}\\\PGvdash u:\PathTy A a b}{
\PGvdash u~@~\lcon \equiv a:A[\lcon/x] \\ \text{and} \\ \PGvdash u~@~\rcon \equiv b:A[\rcon/x]}
\end{mathpar}
We may rephrase these rules using the idea of cofibrations and partial elements:
\begin{itemize}
\item The path type \fbox{$\PathTy A a b$}
is a $\Pi$-type \fbox{$(x:\II)\to A$} carrying a partial element, denoted as:
\[\ExtTy{x}{A}{rll}
{x&\equiv\lcon&\mapsto a\\
x&\equiv\rcon&\mapsto b}\]
...
and an arbitrary partial element, and the rest of the rules will still work.
