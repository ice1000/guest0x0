...
\item A $\conj$ specifies any $n$-cube (for $n\leq 2$) in $\Phi$.
For example, $x\equiv\lcon\land y\equiv\rcon$ specifies a line $v$,
and $x\equiv\lcon\land y\equiv\rcon\land z\equiv\lcon$ specifies a point $\star$:
\carloCTikZ{\carloXyz \refcube{XYLR}}
\item A $\disj$ talks about several $n$-cubes (for $n\leq 2$) in $\Phi$ at the same time.
For example, $x\equiv\lcon\lor x\equiv\rcon$ corresponds to the following two squares:
...
Suppose $\Gvdash\isType{A}$, $\Gvdash u:A$, and $\conj$ is a valid cofibration.
