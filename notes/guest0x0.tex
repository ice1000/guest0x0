...
$\lambda$-calculus should work for the type theory introduced in this tutorial too.
...
Note that these two placements are reflection by the diagonal.
...
Note that all of them can also be reflected by the diagonal.
\end{remark}
...
face ``conditions'' like \fbox{$x\equiv\lcon$} or \fbox{$x\equiv\rcon$}, as in~\cref{fig:cofib}.
\begin{figure}[h!]
\[\begin{array}{rll}
  \cond ::= & x\equiv\lcon \mid x\equiv\rcon & \text{condition} \\
  \conj ::= & \cond \mid \cond \land \conj & \text{conjunction} \\
  \disj ::= & \bot \mid \top \mid \conj \mid \conj \lor \disj & \text{disjunction}
\end{array}\]
...
\item A $\cond$ specifies a $2$-dimensional cube (a square) face in $\Phi$, e.g. \fbox{$x\equiv\lcon$}
...
\fbox{$x\equiv\lcon\land y\equiv\rcon$} specifies a line $v$,
and \fbox{$x\equiv\lcon\land y\equiv\rcon\land z\equiv\lcon$} specifies a point $\star$:
\carloCTikZ{\carloXyz \refcube{XYLR}}
\item A $\disj$ talks about several $n$-cubes (for $n\leq 2$) in $\Phi$ at the same time, e.g.
\fbox{$x\equiv\lcon\lor x\equiv\rcon$} corresponds to the following two squares:
...
(recall~\cref{not:syntax-def}):
\begin{figure}[h!]
\[\begin{array}{rl}
	\face::=&\conj\mapsto u \mid \conj\mapsto u,\face\\
	u,A::=&\lrbbar{\face} \mid \cdots~\text{(other term formers)}
\end{array}\]
...
\end{itemize}
It remains to derive the typing rules for partial elements.

\subsection{Reduction of partial elements}\label{sub:red-cofib}
Consider a well-defined (we will define well-typedness of
partial elements later in~\cref{sub:tyck-cofib}) simple partial element:
\[\LRbbar{\begin{array}{rll}
x\equiv\lcon\mapsto u\\
\conj\mapsto v\\
\end{array}}:\Partial{x\equiv\lcon\lor \conj}A\]
We may substitute $x$ with many possible terms of type $\II$:
\begin{itemize}
\item $\lcon$, i.e. take the face that $x\equiv\lcon$,
then evidently we get the face $u$, so the term should reduce to $u$.
\item $\rcon$, i.e. take the face that $x\equiv\rcon$,
which is unspecified by the first clause, so we remove it and
the term becomes $\lrbbar{\conj\mapsto u}[\rcon/x]:\Partial{\conj}A$.
\end{itemize}
\TODO
...
\end{mathpar}
We want to construct the following partial element:
\carloCTikZ{\carloXyz \refcube{BBFaces}}
Recall~\cref{fig:parEl}, we may directly translate that into:
\[\LRbbar{\begin{array}{rll}
z&\equiv\rcon&\mapsto u~x~y\\
y&\equiv\lcon&\mapsto v~x~z
\end{array}}\]
Note that they share the same line \fbox{$z\equiv\rcon\land y\equiv\lcon$}.
The shared line can be represented as $u~x~\lcon$ or $v~x~\rcon$,
but these two lines are actually the same line,
...
\begin{mathpar}
\inferrule{\Gvdash\isType{A}\\
\PGvdash u~x~y:A\\
\PGvdash v~x~z:A\\
\PGvdash u~x~\lcon\equiv v~x~\rcon}{\Phi\vdash\LRbbar{\begin{array}{rll}
z&\equiv\rcon&\mapsto u~x~y\\
y&\equiv\lcon&\mapsto v~x~z
\end{array}}:\Partial{(z\equiv\rcon\lor y\equiv\lcon)}A}
\end{mathpar}
This seems good enough.
\end{example}

We generalize~\cref{ex:parTyck} to an arbitrary partial element
$\lrbbar{\conj_0\mapsto u_0, \conj_1\mapsto u_1, \cdots, \conj_n\mapsto u_n}$ for $n\in\mathbb{N}$.
We need to find an algorithm that makes sure they agree.
For two faces $\conj_i\mapsto u_i$ and $\conj_j\mapsto u_j$, from~\cref{ex:parTyck} we can tell that
they overlap at $\conj_i\land\conj_j$. In case they do not overlap
\TODO
